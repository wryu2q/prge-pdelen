...
Programet börjar med att definiera vilken storlek fönstret ska ha och vad dess yta ska kallas, därefter definieras vilken FPS det ska vara, sedan definieras vilka färger som ska vara konstanter. Efter det så körs menyn där det dyker upp en massa knappar på skärmen som kör knapp funktionen. Om man väljer att trycka på en kanapp så körs en funktion med des relaterade budskap. om man väljer att köra manual sidan så viasa en text som använder sig av funktionen för att visa grafisk text, samt knappar som avslutar instruktionssidan. om man väljer att köra spelet så kommer en meny för att välja spelläge där man har knappar som kommer börja med att ändra den globala variabeln för svårighetsgrad och sedan köra spelfunktionen. spelfunktionen bildar en tom matris som sedan fylls på med en rekursiv funktion som fyler matrisen med eller lämnar tomrum och hur des kopplingar med varandra är. därefter så fylls laburinten med inehåll så som kål mm. .när laburinten är färdiggenererad så körs en funktion som visar spelplanen. sedan så slumpas spelaren till ett tomt rum på spelplanen, sedan slumpas wumpus på spelplanen. därefter så ritas jägaren ut. efter att det är klart så ritas aktivitetsföltet ut som har knappar för att ändra på var spelaren är eller göra att spelaren skjuter. samt att betrakta kringligande objekt och se var wumpus är så att man kan skriva vad jägaren känner. när spelaren valt så flyttas jägaren och man ser om han hamnade på ett hinder eller wumpus. därefter så flyttas wumpus och man kör om rundan.

om man dör så får man ett medelande och spelfunktionen avslutas och man hamnar i huvudmenyn. om man vinner så komer man till att man har vunnitsidan och man får skriva sitt namn och objektet Stats som samlat införmation om valen man gjort räknar ut ett high score som sedan sparas i ett dokument. Dokumentet sparas med hjälp av pickle som sparar objektet

I menyn kan man även välja att se föregående high scores från dokumentet för highscores med hjälp av shelve som öppnar dom sparade objekten. 
